...
(function($){
	...
	*/
	...
	var wowSlider = $("#wowslider-container").wowSlider({effect: effects || "blinds",prev:"",next:"",fallback:ws_blinds,duration:duration,delay:delay,width:855,height:350,fullScreen:true,autoPlay:true,autoPlayVideo:false,stopOnHover:false,loop:false,bullets:true,caption:true,captionEffect:"move",controls:true});
		...
		if(/laptop|tablet|mobile/g.test(thisClass)) {
			...
			if(/laptop/g.test(thisClass)) {
				sliderCont.css('maxWidth', sliderCont.width()).animate({
					maxWidth: curResponsive>1?$(window).width():855
				}, resizeWnd);
